...
const formatearTienda = (t) => {
  if (!t) return null;
  const diseno = Array.isArray(t.tienda_diseno) ? t.tienda_diseno[0] : t.tienda_diseno;
  return {
    ...t,
    _id: t.id,
...
const toggleActivarTienda = async (req, res) => {
  try {
    const { data: actual } = await supabase.from('tiendas').select('activa').eq('id', req.params.id).single();
    if (!actual) return res.status(404).json({ exito: false, mensaje: 'Tienda no encontrada.' });
...
const toggleDestacarTienda = async (req, res) => {
  try {
    const { data: actual } = await supabase.from('tiendas').select('destacada').eq('id', req.params.id).single();
    if (!actual) return res.status(404).json({ exito: false, mensaje: 'Tienda no encontrada.' });
...
const eliminarTienda = async (req, res) => {
  try {
    const { data: tienda } = await supabase.from('tiendas').select('id, usuario_id').eq('id', req.params.id).single();
...
const obtenerUsuarios = async (req, res) => {
  try {
    ...
    res.json({
      exito: true,
      usuarios: usuarios.map(u => ({ ...u, _id: u.id, createdAt: u.fecha_registro })),
    });
  } catch (error) {
...
const eliminarUsuario = async (req, res) => {
  try {
    const { data: usuario } = await supabase.from('usuarios').select('id, rol').eq('id', req.params.id).single();
    if (!usuario) return res.status(404).json({ exito: false, mensaje: 'Usuario no encontrado.' });
    if (usuario.rol === 'admin') return res.status(403).json({ exito: false, mensaje: 'No puedes eliminar a otro administrador.' });

    if (usuario.rol === 'vendedor') {
      const { data: tienda } = await supabase.from('tiendas').select('id').eq('usuario_id', usuario.id).single();
      if (tienda) {
        const { data: pedidos } = await supabase.from('pedidos').select('id').eq('tienda_id', tienda.id);
        if (pedidos?.length) await supabase.from('detalles_pedido').delete().in('pedido_id', pedidos.map(p => p.id));
        await supabase.from('pedidos').delete().eq('tienda_id', tienda.id);
        await supabase.from('banners').delete().eq('tienda_id', tienda.id);
        await supabase.from('productos').delete().eq('tienda_id', tienda.id);
        await supabase.from('tienda_diseno').delete().eq('tienda_id', tienda.id);
...
// ============================================
// Helper: Normalizar banner para frontend
// ============================================
