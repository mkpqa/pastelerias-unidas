  ...
  if (error) {
    return (
      <div style={{ maxWidth: '600px', margin: '3rem auto', padding: '0 1rem' }}>
        <div style={{ background: '#fde8e8', borderRadius: '12px', padding: '20px', border: '1px solid #e8a0a0', textAlign: 'center' }}>
          <p style={{ fontSize: '14px', color: '#8b2f2f' }}>❌ {error}</p>
  ...
  return (
    <div style={{ maxWidth: '1000px', margin: '0 auto', padding: '1.5rem 1rem 3rem' }}>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1.5rem' }}>
        <div>
          <h1 style={{ fontSize: '22px', color: '#3a1a1a', margin: '0 0 4px' }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '16px', marginBottom: '20px' }}>
        ...
        <div style={{ background: '#fff', borderRadius: '14px', border: '1px solid #e8d5cc', padding: '20px' }}>
          <h3 style={{ fontSize: '14px', color: '#3a1a1a', marginBottom: '16px', display: 'flex', alignItems: 'center', gap: '8px' }}>
            <span style={{ fontSize: '18px' }}>🏪</span> Información de la Tienda
          </h3>

          <div style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>
            {[
              { label: 'Nombre', value: tienda.nombre, icon: '🏷️' },
              { label: 'URL Pública', value: `/tienda/${tienda.slug}`, icon: '🔗' },
              { label: 'Especialidad', value: tienda.especialidad, icon: '🎂' },
              { label: 'Ubicación', value: tienda.ubicacion, icon: '📍' },
              { label: 'Teléfono', value: tienda.telefono, icon: '📞' },
            ].map((item, i) => (
              <div key={i} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '8px 12px', background: '#fdf8f5', borderRadius: '8px' }}>
                <span style={{ fontSize: '12px', color: '#6b4c4c' }}>{item.icon} {item.label}</span>
          ...
          {tienda.descripcion && (
            <div style={{ marginTop: '12px', padding: '10px 12px', background: '#fdf8f5', borderRadius: '8px' }}>
              <span style={{ fontSize: '11px', color: '#6b4c4c' }}>📝 Descripción</span>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: '12px', marginBottom: '20px' }}>
        {[
          { label: 'Productos', value: String(totalProductos), icon: '📦', desc: 'en tu catálogo' },
          { label: 'Pedidos', value: '0', icon: '🛒', desc: 'recibidos' },
          { label: 'Código Pedido', value: `PED-${String(tienda.contadorPedidos + 1).padStart(3, '0')}`, icon: '🔢', desc: 'próximo número' },
        ].map((stat, i) => (
      ...
      <div style={{ background: '#fff', borderRadius: '14px', border: '1px solid #e8d5cc', padding: '20px' }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '14px' }}>
          <h3 style={{ fontSize: '14px', color: '#3a1a1a', margin: 0, display: 'flex', alignItems: 'center', gap: '8px' }}>
            <span style={{ fontSize: '18px' }}>👁️</span> Así se ve tu tienda — <strong style={{ color }}>{plantillasLabel[plantilla]}</strong>
          </h3>
